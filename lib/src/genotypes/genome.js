...
class Genome {
    constructor(options, sequence = index_1.values(options.genomeLength * options.nucleotideLength), idLength = Math.min(12, options.genomeLength)) {
    ...
    get id() {
        ...
        letters = _
            ...
            .map((n) => n.reduce((a, b) => new index_1.Nucleotide((a.value + b.value) / 2)) //average nucleotides)
        )
    ...
    get nucleotides() {
        let nucleos = _
            ...
            .map(n => new index_1.Nucleotide(n));
        return nucleos;
    }
