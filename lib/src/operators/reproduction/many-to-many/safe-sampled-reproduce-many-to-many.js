...
function safeSampledReproduceManyToMany(genomes, n, fitness, sampleSize = 5, weights = _.range(0, genomes.length).map(i => value_1.value())) {
    let result = [];
    let avgFit = avg_fitness_1.avgFitness(genomes, fitness);
    while (result.length < n) {
        result = _.concat(result, sampled_reproduce_many_to_many_1.sampledReproduceManyToMany(genomes, n, fitness, sampleSize, weights)
