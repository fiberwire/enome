    ...
    reproduce(n = 1) {
        if (this.genomes.length + n > this.popOptions.maxSixe) {
            if (n == 1) {
                ...
                g = mutate_1.mutate(g, this.popOptions.mutateOptions.mutateChance, this.popOptions.mutateOptions.mutateType);
                this.dequeue();
                this.add(g);
            }
            else {
                let gs = reproduce_many_to_many_1.reproduceManyToMany(this.genomes, n);
                gs = mutate_many_1.mutateMany(gs, this.popOptions.mutateOptions.mutateChance, this.popOptions.mutateOptions.mutateType);
