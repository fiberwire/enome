...
export class SumOrganism extends Organism<
  ISumGenomeOptions,
  ISumPopOptions,
  ISumOrganismOptions,
  ISumData,
...
> {
  public createPhenotype(genome: Genome<ISumGenomeOptions>): number[] {
    return _.range(genome.options.length).map(i =>
      genome.g.int(genome.options.min, genome.options.max)
    );
  }
  ...
  public observe(
    interaction: IInteraction<ISumEnvState, ISumEnvState>
  ): ISumData {
